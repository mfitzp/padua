...
def impute_from_gaussian(df, width=[0.3], down_shift=[1.8], prefix="Intensity "):
    ...
    :param df:
    :param width: Scale factor for the imputed distibution relative to the standard deviation of measured values
    :param down_shift: Shift the imputed values down, in units of std. dev.
...
#    df = df.fillna(0.0)
    ...
    if len(width)>1 and len(mycols)!=len(width):
        print "Length of iterable 'width' does not match # of columns"
    if len(down_shift)>1 and len(mycols)!=len(down_shift):
        print "Length of iterable 'down_shift' does not match # of columns"
            
    print df[mycols[0]].mean(axis=0)
